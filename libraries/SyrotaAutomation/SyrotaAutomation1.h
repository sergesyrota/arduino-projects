...
public:
    ...
    void sendResponse(const char *raw); // Sends raw response string
    boolean assertCommand(const char *command); // compares if command in parameters matches command in buffer
    boolean assertCommandStarts(const char *command, char *paramString); // compares if command in buffer starts with what's passed in parameters
    void responseStart(); // Sends response headers
    void responseSendPart(const char *raw); // Sends response packet. If response has not been started, it will do that first.
